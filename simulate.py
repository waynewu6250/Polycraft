...
from env import EnvHandler, PolycraftEnv
import PolycraftGym
...
def train_agent(game = 10, scene = './experiments/hgv1_1.json'):

    env = PolycraftEnv(height=256, width=256, color=True)
    # observation = env.reset()
    # obs, reward, done = env.step(1)

    # env_handler = EnvHandler(scene, game)

    
    # Create Graph and agent
    tf.reset_default_graph()
    sess = tf.InteractiveSession()
    agent = Agent("agent", env.observation_space.shape, len(env.action_dic))
    ...
    batch_states = batch_states[np.newaxis, :]
    entropy_history = []
    rewards_history = []
    ...
    for i in trange(opt.num_global_steps): 
        ...
        batch_next_states, batch_rewards, batch_done = env.step(batch_actions[0])
        print(batch_rewards)
        feed_dict = {
            agent.states_ph: batch_states,
            agent.actions_ph: batch_actions,
            agent.next_states_ph: batch_next_states[np.newaxis, :],
            agent.rewards_ph: [batch_rewards],
            agent.is_done_ph: [batch_done],
        }
        batch_states = batch_next_states[np.newaxis, :]
...
if __name__ == "__main__":
    ...
    scene = './experiments/hgv1_1.json'

    if args.mode == "play":
        global_rewards = forward_run(scene)
    elif args.mode == "train":
        train_agent(game=args.game, scene=scene)
    else:
